...
from app.viewmodels.services.TradingBot import KrakenTradingBot
...
class TradingBotManager:
    ...
    @classmethod
    def start_bot(cls, user_id, kraken_api: KrakenAPI, config):
        """
        Start a trading bot for the specified user using the KrakenTradingBot implementation
        ...
        try:
            ...
            # Create and start the KrakenTradingBot
            try:
                logger.debug(f"Initializing KrakenTradingBot for user {user_id}")
                bot = KrakenTradingBot(user_id, kraken_api, api_key, api_secret, config)
                
                logger.debug(f"Starting KrakenTradingBot for user {user_id}")
                if not bot.start():
                ...
                logger.error(f"Error initializing KrakenTradingBot for user {user_id}: {str(e)}")
    ...
    def start_legacy_bot(cls, user_id, kraken_api: KrakenAPI, config):
        ...
        try:
            ...
            bot_thread = Thread(
                ...
                args=(current_app._get_current_object(), kraken_api, api_key, api_secret, config, stop_event),
                daemon=True
            )
    ...
    def stop_legacy_bot(cls, user_id):
        ...
        try:
            ...
            if bot_data['thread'].is_alive():
                logger.warning(f"Legacy bot thread for user {user_id} didn't stop gracefully")
...
def run_trading_bot(app, kraken_api: KrakenAPI, api_key: str, api_secret: str, config, stop_event):
    """
